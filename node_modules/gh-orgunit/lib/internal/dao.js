...
var exportOrgUnit = module.exports.exportOrgUnit = function(orgUnit, format, callback) {
    ...
    DB.OrgUnit.findAll(options).complete(function(err, children) {
        ...
        if (format === 'csv') {
            return exportOrgUnitCSV(children, callback);
        } else {
            return exportOrgUnitJSON(children, callback);
        }
    });
};

var exportOrgUnitCSV = function(children, callback) {
    // Get the simplified tree by exporting it to JSON first
    exportOrgUnitJSON(children, function(err, jsonData) {
        if (err) {
            return callback(err);
        }
        ...
        // Now convert the json tree to flat CSV data
        csv.transform(jsonData, function(data) {
            ...
            console.log('Done');
            console.log(arguments);
            csv.stringify(output, callback);
        });
    });
};

var exportOrgUnitJSON = function(children, callback) {
    var data = [];
    _.each (children, function(child) {
        ...
        // Export the organsiational unit's series
        ...
        data.push(exportedChildOrgUnit);
    });
