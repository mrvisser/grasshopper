...
var exportOrgUnit = module.exports.exportOrgUnit = function(orgUnit, format, callback) {
    // Get the event series and events under the organisational unit
    var options = {
        'where': {
            'ParentId': orgUnit.id
        },
    ...
    DB.OrgUnit.findAll(options).complete(function(err, children) {
        if (err) {
            log().error({
                'err': err,
                'id': orgUnit.id
        ...
        if (format === 'csv') {
            return exportOrgUnitCSV(orgUnit, children, callback);
        } else {
            return exportOrgUnitJSON(orgUnit, children, callback);
        }
    });
};

/**
 * Export the direct organisational units, event series and events under an organisational unit to CSV
 *
 * @param  {OrgUnit}        orgUnit             The organisational unit to export
 * @param  {OrgUnit[]}      children            The direct children of the organisational unit, including the series and events
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 * @param  {String}         callback.data       The exported organisational units, event series and events as CSV
 */
var exportOrgUnitCSV = function(orgUnit, children, callback) {
    // Get the simplified tree by exporting it to JSON first
    exportOrgUnitJSON(orgUnit, children, function(err, exportedOrgUnit) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(exportedOrgUnit.children, function(childOrgUnit) {
            _.each(childOrgUnit.series, function(series) {
                _.each(series.events, function(event) {
                    var record = {
                        'orgUnitId': orgUnit.id,
                        'orgUnitExternalId': orgUnit.externalId,
                        'orgUnitDisplayName': orgUnit.displayName,
                        'orgUnitDescription': orgUnit.description,
                        'orgUnitPublished': orgUnit.published,
                        'childOrgUnitId': childOrgUnit.id,
                        'childOrgUnitExternalId': childOrgUnit.externalId,
                        'childOrgUnitDisplayName': childOrgUnit.displayName,
                        'childOrgUnitDescription': childOrgUnit.description,
                        'childOrgUnitPublished': childOrgUnit.published,
                        'seriesId': series.id,
                        ...
                        'eventOrganiserShibbolethID': null,
                        'eventOrganiserDisplayName': null
                    };
                    ...
                    if (_.isEmpty(event.organisers)) {
                        ...
                        _.each(event.organisers, function(organiser) {
                            var organiserRecord = _.clone(record);
                            if (_.isObject(organiser)) {
                                organiserRecord.eventOrganiserId = organiser.id;
                                organiserRecord.eventOrganiserShibbolethID = organiser.shibbolethId;
                                organiserRecord.eventOrganiserDisplayName = organiser.displayName;
                            } else {
                                organiserRecord.eventOrganiserDisplayName = organiser;
                            }

                            records.push(organiserRecord);
                        });
                    }
                });
            });
        ...
        var options = {
            ...
            'columns': [
                ...
                'orgUnitPublished',
                'childOrgUnitId',
                'childOrgUnitExternalId',
                'childOrgUnitDisplayName',
                'childOrgUnitDescription',
                'childOrgUnitPublished',
                'seriesId',
                'seriesExternalId',
                'seriesDisplayName',
                'seriesDescription',
                ...
                'eventEnd',
                'eventOrganiserId',
                'eventOrganiserShibbolethID',
                'eventOrganiserDisplayName'
            ]
        };
 ...
 */
var exportOrgUnitJSON = function(orgUnit, children, callback) {
    var exportedOrgUnit = {
        'id': orgUnit.id,
    ...
    _.each(children, function(child) {
        var exportedChildOrgUnit = {};
        exportedChildOrgUnit.id = child.id;
        exportedChildOrgUnit.externalId = child.externalId;
        exportedChildOrgUnit.displayName = child.displayName;
        exportedChildOrgUnit.description = child.description;
        exportedChildOrgUnit.published = child.published;

        // Export the organisational unit's series
        exportedChildOrgUnit.series = [];
        _.each(child.Series, function(series) {
            var exportedSeries = {};
            exportedSeries.id = series.id;
            exportedSeries.externalId = series.externalId;
            exportedSeries.displayName = series.displayName;
            exportedSeries.description = series.description;

            // Export the events of the series
            exportedSeries.events = [];
            _.each(series.Events, function(event) {
                // Stringify the event instance so we have access to the `organisers` property
                event = event.toJSON();

                var exportedEvent = {};
                exportedEvent.id = event.id;
                exportedEvent.externalId = event.externalId;
                exportedEvent.displayName = event.displayName;
                exportedEvent.description = event.description;
                exportedEvent.notes = event.notes;
                exportedEvent.location = event.location;
                exportedEvent.start = event.start;
                exportedEvent.end = event.end;

                exportedEvent.organisers = [];
                _.each(event.organisers, function(organiser) {
                    if (_.isString(organiser)) {
                        exportedEvent.organisers.push(organiser);
                    } else {
                        ...
                        exportedEvent.organisers.push(exportedOrganiser);
                    }
                });

                exportedSeries.events.push(exportedEvent);
            });
            exportedChildOrgUnit.series.push(exportedSeries);
        });

        exportedOrgUnit.children.push(exportedChildOrgUnit);
    });

    return callback(null, exportedOrgUnit);
};
