...
/**
 * Assert that the direct organisational units, event series and events under an organisational unit can be exported
 *
 * @param  {RestClient}         client                  The REST client to make the request with
...
var assertExportOrgUnit = module.exports.assertExportOrgUnit = function(client, id, format, callback) {
    assertGetOrgUnit(client, id, false, function(orgUnit) {
        ...
        // client, app, includeSeries, includePermissions, parent, type, callback
        assertGetOrgUnits(client, orgUnit.AppId, true, false, id, null, function(data) {
            var childOrgUnits = data.results;

            var series = [];
            _.each(childOrgUnits, function(childOrgUnit) {
                var seriesIds = _.pluck(childOrgUnit.Series, 'id');
                series = _.union(series, seriesIds);
            });

            // Get the events for all the series
            SeriesTestsUtil.getAllSeriesEvents(client, series, function(eventsPerSeriesId) {

                // Export the organisational units
                client.orgUnit.exportOrgUnit(id, format, function(err, data, response) {
                    assert.ok(!err);

                    if (format === 'csv') {
                        assert.strictEqual(response.headers['content-type'], 'text/csv; charset=utf-8');

                        // Assert the csv can be parsed
                        ...
                        assert.strictEqual(data.id, orgUnit.id);
                        assert.strictEqual(data.externalId, orgUnit.externalId);
                        assert.strictEqual(data.displayName, orgUnit.displayName);
                        assert.strictEqual(data.description, orgUnit.description);
                        assert.strictEqual(data.published, orgUnit.published);

                        // Assert the correct children are returned
                        assert.strictEqual(data.children.length, childOrgUnits.length);
                        _.each(data.children, function(exportedChildOrgUnit) {
                            // Assert the exported child organisational unit is one we expect
                            var retrievedChildOrgUnit = _.find(childOrgUnits, {'id': exportedChildOrgUnit.id});
                            assert.ok(retrievedChildOrgUnit);

                            // Assert the exported child organisational unit's properties
                            assert.strictEqual(exportedChildOrgUnit.id, retrievedChildOrgUnit.id);
                            assert.strictEqual(exportedChildOrgUnit.externalId, retrievedChildOrgUnit.externalId);
                            assert.strictEqual(exportedChildOrgUnit.displayName, retrievedChildOrgUnit.displayName);
                            assert.strictEqual(exportedChildOrgUnit.description, retrievedChildOrgUnit.description);
                            assert.strictEqual(exportedChildOrgUnit.published, retrievedChildOrgUnit.published);

                            // Assert the correct series are present
                            assert.strictEqual(exportedChildOrgUnit.series.length, retrievedChildOrgUnit.Series.length);
                            _.each(exportedChildOrgUnit.series, function(exportedSeries) {
                                // Assert the exported series is one we expect
                                var retrievedSeries = _.find(retrievedChildOrgUnit.Series, {'id': exportedSeries.id});
                                assert.ok(retrievedSeries);

                                // Assert the series properties
                                assert.strictEqual(exportedSeries.id, retrievedSeries.id);
                                assert.strictEqual(exportedSeries.externalId, retrievedSeries.externalId);
                                assert.strictEqual(exportedSeries.displayName, retrievedSeries.displayName);
                                assert.strictEqual(exportedSeries.description, retrievedSeries.description);

                                // Assert the correct events are present
                                var retrievedEvents = eventsPerSeriesId[retrievedSeries.id];
                                assert.strictEqual(exportedSeries.events.length, retrievedEvents.length);
                                _.each(exportedSeries.events, function(event) {
                                    var retrievedEvent = _.find(retrievedEvents, {'id': event.id});
                                    assert.ok(retrievedEvent);
                                    assert.strictEqual(event.id, retrievedEvent.id);
                                    assert.strictEqual(event.externalId, retrievedEvent.externalId);
                                    assert.strictEqual(event.displayName, retrievedEvent.displayName);
                                    assert.strictEqual(event.description, retrievedEvent.description);
                                    assert.strictEqual(event.start, retrievedEvent.start);
                                    assert.strictEqual(event.end, retrievedEvent.end);
                                    assert.strictEqual(event.notes, retrievedEvent.notes);
                                    assert.strictEqual(event.location, retrievedEvent.location);
                                    assert.deepEqual(event.organisers, retrievedEvent.organisers);
                                });
                            });
                        });
