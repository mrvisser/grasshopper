...
var getServiceProviderUrl = module.exports.getServiceProviderUrl = function(ctx) {
    ...
    var data = {'appId': ctx.app.id};
    var signature = Signature.createExpiringSignature(data, 60, 60);

    // Create and return the full URL
    return util.format('%s/api/auth/shibboleth/sp?appId=%s&signature=%s&expires=%s', spURL, ctx.app.id, signature.signature, signature.expires);
};
 ...
 *
 * @param  {Number}     appId               The id of the application to validate and retrieve
 ...
 */
var validateInitiateParameters = module.exports.validateInitiateParameters = function(appId, signature, expires, callback) {
    var validator = new Validator();
    validator.check(appId, {'code': 400, 'msg': 'Missing tenant alias parameter'}).notEmpty(appId);
    ...
    return getShibbolethEnabledApplication(appId, callback);
};

/**
 * Given an `appId`, get the full app object. This method
 * will perform some extra validation such as checking whether an application
 * with that id exists and whether Shibboleth is enabled for
 * the specified application
 *
 * @param  {Number}     appId               The id of the application to retrieve
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error object, if any
 * @param  {App}        callback.app        The full app object for the given id
 */
var getShibbolethEnabledApplication = module.exports.getShibbolethEnabledApplication = function(appId, callback) {
 ...
 */
var getUser = module.exports.getUser = function(app, userId, signature, expires, callback) {
    var validator = new Validator();
    validator.check(userId, {'code': 400, 'msg': 'Missing user id parameter'}).notEmpty(userId);
    validator.check(userId, {'code': 400, 'msg': 'Invalid user id parameter'}).isNumeric();
    validator.check(signature, {'code': 400, 'msg': 'Missing signature parameter'}).notEmpty(signature);
    validator.check(expires, {'code': 400, 'msg': 'Missing expires parameter'}).notEmpty(expires);
    validator.check(expires, {'code': 400, 'msg': 'Invalid expires parameter'}).isNumeric();
    validator.check(expires, {'code': 400, 'msg': 'Invalid expires parameter'}).min(Date.now());
    ...
    UsersDAO.getUser(userId, callback);
};
